...
void initmultiplayers(char damultioption, char dacomrateoption, char dapriority)
{
    ...
    if (gcom == NULL)
	{
        iprintf("Network transport initialization failed. Aborting...\n");
        exit(1);
	}
...
typedef enum {
	SO_REUSEADDR=0x0004, SO_LINGER=0x0080, SO_DEBUG,
   SO_KEEPALIVE, SO_DONTROUTE, SO_BROADCAST, SO_OOBINLINE,
   SO_SNDBUF, SO_RCVBUF, SO_ERROR, SO_TYPE
   } SocketOption;
typedef enum {SOL_SOCKET=0xffff} SocketLevel;
#define IPPROTO_UDP     17
#define IPPROTO_IP      0x100
